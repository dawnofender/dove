...
class Thingy : public Thing, public Node<Thingy, std::vector<Thingy>> {
...
public:
    ...
    bool addComponent(std::string && typeName);
    ...
    std::shared_ptr<Thingy> getRoot();

    std::shared_ptr<Thingy> getParent();
    ...
    // Output: a ptr to the found thingy.
    template<class ThingyType>
    std::shared_ptr<ThingyType> getChild();
    
    // getChild finds a child thingy of the given type.
    // Output: a reference to the found thingy.
    // Inputs: the name of the child
    std::shared_ptr<Thingy> getChild(std::string childName);
    
    void setParent(std::shared_ptr<Thingy> newParent);

    void setName(std::string && newName);
    std::string getName();

    // remove 1 child by pointer
    bool removeChild(std::shared_ptr<Thingy> &child);
    bool removeChild(Thingy *&child);
    ...
    template<class ThingyType>
    std::vector<std::shared_ptr<Thingy>> getChildren();
...
protected:
    ...
    mutable std::weak_ptr<Thingy> cachedRoot; // mutable to allow caching in getRoot(), not implemented yet
