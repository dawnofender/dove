...
public: 
    ...
    void generateOctree(Cell* cell, int8_t cellDepth, vec3 cellPos, float cellSize) {
        float childSize = cellSize/2.f;
        vec3 blockPos = cellPos + vec3(childSize);

        if(cellDepth > 0 && distance(cellPos, camPos)/cellSize < lod) {
            cell->leaf = false;
            ...
            for (int i = 0; i < 8; ++i) {
                ...
                generateOctree(cell->children[i].get(), cellDepth-1, childPos, childSize);
            }
            ...
            cellTree.leafMap.insert({std::shared_ptr<Cell>(cell), {cellPos, cellDepth}});
        }
    }
    

    void indexLeaves(Cell* cell, int8_t cellDepth, vec3 cellPos, float cellSize) {
        for (auto& child: cell->children) {
            if(child) {
                // calculate position
                ...
                if(child->leaf) {
                    cellTree.leafMap.insert({std::shared_ptr<Cell>(child), {childPos, depth-1}});
    ...
    void indexLeaves(Cell* cell, int8_t cellDepth, vec3 cellPos, float cellSize) {
        ...
        if(cellDepth > 0 && distance(cellPos, camPos)/cellSize < lod) {
            for (int i = 0; i < 8; ++i) {
                ...
                indexLeaves(cell->children[i].get(), cellDepth-1, childPos, childSize);
            }
            
        } else cellTree.leafMap.insert({std::shared_ptr<Cell>(cell), {cellPos, cellDepth}});
    }
    void generateMatter(Cell* cell, vec3 blockPos) { //if(cdepth > 0 && distance3d(pos, lodpos)/csize < lod*2) { float childSize = cellSize/2.f; vec3 blockPos = cellPos + vec3(childSize);
    ...
    void buildMeshData() {
        for(const auto& leafData : cellTree.leafMap) {
            //leafData.first -> cell
            //leafData.second -> cell data (position, scale, matter, etc.)
            
            // currently only rendering solid blocks
            if(!leafData.first->transparent) {
                for(int i = 0; i < 6; i++) {
                    if(leafData.first->neighbors[i] && leafData.first->neighbors[i]->transparent) {
                        ...
                        for(int j = 0; j < 4; j++) {
                            mesh.vertices[vertexInd+j] = leafData.second.position + cellTree.sizes[leafData.second.depth] * cube.vertices[cubeInd+j];
                        }
